...
const createRide=async(req,res)=>{try  { 
...
const searchRide=async(req,res)=>{
    try {
        ...
        const rides = All_rides.filter(ride => {
            ...
            const srcDistance = turf.pointToLineDistance(srcPt, line, { units: 'kilometers' });
            const destDistance = turf.pointToLineDistance(destPt, line, { units: 'kilometers' });
